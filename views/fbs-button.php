...
*/
	...
	//sub buttons only from specific types (click or hover)
	if ($type == '1' || $type == '2' || $type == '3') {
	if(is_array($subbutton[$number]) && count($subbutton[$number]) > 0):
	$i = 0;
		foreach($subbutton[$number] as $fbs_sub_btn):
				
				list($sub_type, $sub_title, $sub_link, $sub_image, $sub_color, $sub_bg_color) = $fbs_sub_btn; 
            ...
            ?>
            <a id="fbs-btn-<?php echo $i; ?>" data-id="<?php echo $i; ?>" title="<?php esc_html_e($sub_title) ?>" href="javascript: void(0);" class="fbs-buttons fbs-secondary">
                <img src="<?php esc_html_e($sub_image) ?>" style="color: <?php esc_html_e($sub_color) ?> !important; background-color: <?php esc_html_e($sub_bg_color) ?> !important;">
			</a>
		<?php 
		$i++;
		endforeach;	
				...
				if ($type == '0' || $type == '4') echo '<a href="#" class="fbs_close">x</a>';
...
// SUB BUTTONS ONLY
				...
				if(is_array($subbutton[$number]) && count($subbutton[$number]) > 0):
					...
					foreach($subbutton[$number] as $fbs_sub_btn): 
						...
						list($sub_type, $sub_title, $link, $image, $color, $bg_color) = $fbs_sub_btn; 
...
*/
					...
					<?php else: ?>    
						<div data-parent="<?php echo $number; ?>" style="background-color: display: none; <?php esc_html_e($bg_color) ?>; color: <?php esc_html_e($color) ?>" data-id="<?php echo $i; ?>" class="<?php echo $pos; ?> fbs-box"> 
							<div class="fbs-box-head" style="background-color: <?php esc_html_e($bg_color) ?>; color: <?php esc_html_e($color) ?>; position: relative;">
								<div style="display: inline-block; width: 30px;">
									<span style="position: absolute; display: block; background-image: url('<?php esc_html_e($image) ?>'); width: 27px; height: 27px; background-size: cover; background-repeat: no-repeat; background-position: center; margin-top: -20px;"></span>
								</div>
								<?php esc_html_e($sub_title, 'floating-buttons'); ?>
								<span data-id="<?php echo $i; ?>" class="fbs-box-hide">x</span>
							</div>
							<div class="fbs-body" style="padding: 20px; font-family: courier;">
					<?php if($sub_type == 'whatsapp'): ?>
									<?php
										$number = $content;
										$invalid_array = array(' ', '+', '-', '(', ')', '{', '}', '[', ']');
					...
					<?php elseif($sub_type == 'email'): ?>
									<a href="mailto:<?php esc_html_e($content, 'floating-buttons'); ?>" style="display: block; text-align: center; margin-top: 10px; font-size: 20px; color: <?php esc_html_e($bg_color) ?>;">
										<?php esc_html_e($content, 'floating-action-button'); ?>
									</a>
					<?php elseif($sub_type == 'viber'): ?>
					...
					<?php elseif($sub_type == 'snapchat'): ?>
									<div style="width: 300px; text-align: center;">
										<object data="https://app.snapchat.com/web/deeplink/snapcode?username=<?php esc_html_e($content, 'floating-buttons'); ?>&type=SVG&bitmoji=enable" width="200px" height="200px"></object>
										<div style="display: block; text-align: center; margin-top: 10px; font-size: 24px; color: <?php esc_html_e($bg_color) ?>;">
											<?php esc_html_e($content, 'floating-buttons'); ?>
										</div>
									</div>
					<?php elseif($sub_type == 'line'): ?>
